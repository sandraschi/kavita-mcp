...
# PyInstaller spec — produce kavita-mcp-backend.exe for Tauri NSIS embedding.
# Fleet standard: strip=False, upx=False, noarchive=True (see tauri_nsis_building.md).
# Usage (from repo root):
#   uv run pyinstaller kavita-mcp-backend.spec --distpath dist --clean --noconfirm

block_cipher = None

a = Analysis(
    ["src/kavita_mcp/__main__.py"],
    pathex=["src"],
    binaries=[],
    datas=[("src/kavita_mcp", "kavita_mcp")],
    hiddenimports=[
        "uvicorn.logging",
        "uvicorn.loops",
